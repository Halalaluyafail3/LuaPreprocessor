...
By default, some platform specific extensions may be used if they are defined. If this needs to be avoided, define the macro `LUA_PREPROCESSOR_AVOID_EXTENSIONS` (even a definition for zero is allowed). For example: `-DLUA_PREPROCESSOR_AVOID_EXTENSIONS`. This has no effect on Lua itself, to make Lua itself avoid using extensions it is necessary to define the appropriate macros that Lua accepts.

Additional libraries can be brought into the preprocessor, this can be done by modifying `Libraries.c` to load said libraries. To change the version of Lua used in the preprocessor, replace the folder in `src` that contains the current version of Lua, and update the includes in the files in `src` to use the new name. Some of the functions used may not exist in older versions of Lua.
...
The first line will not be skipped if it starts with an octothorpe `#`, which Lua normally does when loading a file to support shebangs. If the encoding used is ASCII, or is equivalent to ASCII for U+0007..U+000D and U+0020..U+007E then a UTF-8 byte order mark will be skipped if present at the start of a file. A UTF-8 byte order mark consists of three bytes with the following values in decimal and in this order: 239, 187, 191.
...
$if(true){}else{}elseif(){}else{}end -- nothing, and the elseif does not need anything in the condition when not being checked
...
```
* `tostring`: Expects a bracketed sequence of tokens after the string literal or name `tostring` which are converted into a string literal. The specific format should not be relied upon, tokens may have multiple ways of being converted into the string, and the way spaces are inserted should not be relied upon. The symbol `$`, the string literal or name `tostring`, and the bracketed sequence of tokens (including the enclosing brackets) are replaced by the string representation of the tokens within the brackets as a string literal.
```lua
-- the examples here give possible results, the results are not guaranteed to be specifically this
...
```
* `notnow`: Adds 'not nows' to tokens. The brackets in this description are used to indicate optionality, for example `[number]` means a number may or may not appear. In this description `number` refers to a nonnegative integer literal, or a floating point literal that is exactly representable as a nonnegative integer. This macro is overloaded to do multiple things and shall have one of the following forms after the `$` token (`notnow` represents the string literal or name `notnow`):
    ...
    * `notnow [number] : symbol`: Adds the specified number of 'not nows' to the symbol, or one if no number is specified. The symbol `$`, the string literal or name `notnow`, the number (if specified), and the symbol `:` will be removed.
